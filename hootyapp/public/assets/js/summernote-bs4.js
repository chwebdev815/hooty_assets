...
(function(global, factory) {
    ...
    var dialog = renderer.create('<div class="modal" aria-hidden="false" tabindex="-1" role="dialog"/>', function($node, options) {
        ...
        $node.html(['<div class="modal-dialog ' + options.dialogSize + '">', '  <div class="modal-content">', options.title ? '    <div class="modal-header">' + '      <h4 class="modal-title">' + options.title + '</h4>' + '      <button type="button" class="close" data-dismiss="modal" aria-label="Close" aria-hidden="true">&times;</button>' + '    </div>' : '', '    <div class="modal-body">' + options.body + '</div>', options.footer ? '    <div class="modal-footer">' + options.footer + '</div>' : '', '  </div>', '</div>'].join(''))
    })
    ...
    $$1.extend($$1.summernote.lang, {
        'en-US': {
            ...
            video: {
                ...
                insert: 'Insert Video',
                url: 'Video URL',
                providers: '(YouTube, Instagram, or DailyMotion)'
            },
            link: {
                link: 'Link',
                insert: 'Insert Link',
    ...
    var VideoDialog =
        ...
        (function() {
            ...
            VideoDialog.prototype.initialize = function() {
                var $container = this.options.dialogsInBody ? this.$body : this.$editor
                var body = $('.video-modal').length ? $('.video-modal').html().replace('d-none', '') : undefined;
            ...
            VideoDialog.prototype.createVideoNode = function(url, img) {
                ...
                if (ytMatch && ytMatch[1].length === 11) {
                    ...
                    $video = $$1('<iframe>').attr('frameborder', 0).attr('src', '//www.youtube.com/embed/' + youtubeId).attr('width', '640').attr('height', '360').attr('style', 'max-width:100%')
                    $href = $$1('<a>').attr('href', '//www.youtube.com/embed/' + youtubeId);
                    $img = $$1('<img>').attr('src', 'https://img.youtube.com/vi/' + youtubeId + '/hqdefault.jpg').attr('class', 'd-none')
                } else if (igMatch && igMatch[0].length) {
                    $video = $$1('<iframe>').attr('frameborder', 0).attr('src', 'https://instagram.com/p/' + igMatch[1] + '/embed/').attr('width', '612').attr('height', '710').attr('scrolling', 'no').attr('allowtransparency', 'true').attr('style', 'max-width:100%')
                    $href = $$1('<a>').attr('href', 'https://instagram.com/p/' + igMatch[1] + '/embed/');
                    $img = $$1('<img>').attr('src', 'https://www.instagram.com/p/' + igMatch[1] + '/media/?size=l').attr('class', 'd-none')
                    ...
                    $video = $$1('<iframe>').attr('frameborder', 0).attr('src', '//www.dailymotion.com/embed/video/' + dmMatch[2]).attr('width', '640').attr('height', '360').attr('style', 'max-width:100%')
                    $href = $$1('<a>').attr('href', '//www.dailymotion.com/embed/video/' + dmMatch[2]);
                    $img = $$1('<img>').attr('src', 'https://www.dailymotion.com/thumbnail/video/' + dmMatch[2]).attr('class', 'd-none')
                    ...
                    $img = $$1('<img>').attr('src', img);
                    $href = $$1('<a>').attr('href', url);
                    // $img = $$1('<img>').attr('src', img).attr('class', 'd-none')
                } else {
                ...
                var $videoCaption = $('<p class="d-none" style="font-size:12px">Click on the video to view</p>')
                var $imgWithLink = $href.append($img);
                var $output;
                ...
                if ($video && $img) {
                    ...
                    $output = $('<div></div>').append($video).append($imgWithLink).append($videoCaption);
                } else {
                    $output = $('<div></div>').append($imgWithLink).append($videoCaption);
                }
            ...
            VideoDialog.prototype.show = function() {
                    ...
                    this.showVideoDialog(text).then(function(data) {
                        ...
                        var $node = _this.createVideoNode(data.url, data.img)
                ...
                return $$1.Deferred(function(deferred) {
                    ...
                    _this.ui.onDialogShown(_this.$dialog, function() {
                        ...
                        $('.video-item').click(function() {
                            ...
                            _this.ui.toggleBtn($videoBtn, videourl)
                        })
                        ...
                        $videoBtn.click(function(event) {
                            ...
                            deferred.resolve({ url: videourl, img: imgurl })
                        })
